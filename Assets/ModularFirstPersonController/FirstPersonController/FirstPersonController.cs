...
public class FirstPersonController : MonoBehaviour
{
    ...
    public KeyCode scaleDownKey = KeyCode.Q;
    public KeyCode scaleUpKey = KeyCode.E;
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            {
                targetVelocity = transform.TransformDirection(targetVelocity) * currentRunningSpeed;
            ...
            else
            {
                ...
                targetVelocity = transform.TransformDirection(targetVelocity) * currentWalkingSpeed;
    ...
    private float FOVScaleFactor = 150f;
    private float minFOV = 85f; 
    private float maxFOV = 110f;
    private float scalingSpeed = 0.7f;

    // private void AdjustSpeeds()
    // {
    //     float scaleFactor = transform.localScale.x / originalScale.x;
    //     if (scaleFactor <= 0.1)
    //     {
    //         currentWalkingSpeed = walkSpeed / scaleFactor / 5;
    //         currentRunningSpeed = sprintSpeed / scaleFactor / 5;
    //     }


    //     Debug.Log($"SCALE FACTOR :{scaleFactor}");
    //     Debug.Log("CURRENT RUNNING SPEED: " + currentRunningSpeed);
    ...
    {
        float scaleFactor = transform.localScale.x / originalScale.x;
        jumpPower = baseJumpPower * Mathf.Max(scaleFactor, 0.5f); // Ensure a minimum scale factor
    }

    private void AdjustFOV()
    {
        float scaleFactor = transform.localScale.x / originalScale.x;
        float targetFOV = Mathf.Lerp(minFOV, maxFOV, 1 - scaleFactor);

        fov = Mathf.MoveTowards(playerCamera.fieldOfView, targetFOV, FOVScaleFactor * Time.deltaTime);
    }


    private void ScaleUpFunc()
    {
        scaleAmount = Mathf.Clamp(transform.localScale.x * (1 + scalingSpeed * Time.deltaTime), 0.1f, originalScale.x);
        transform.localScale = new Vector3(scaleAmount, scaleAmount, scaleAmount);
        AdjustFOV();
    }

    private void ScaleDownFunc()
    {
        scaleAmount = Mathf.Clamp(transform.localScale.x * (1 - scalingSpeed * Time.deltaTime), 0.1f, originalScale.x);
        transform.localScale = new Vector3(scaleAmount, scaleAmount, scaleAmount);
        AdjustFOV();
